...
def fitness_note_length(chromosome):
    """
        Calculates the decrease (of increase) in fitness due to the amount of 
        rest in the music.
    """
    rest =  chromosome.count("0")
    aanhouding = chromosome.count("F")
    total_rest = chromosome.count("F") + chromosome.count("0")
    if total_rest < 3:
        total_rest = 7 - rest
    if rest is 1 and aanhouding is 2:
        total_rest = 0
    
    return - (total_rest * punishment_rest)


def fitness_note_after_instable_tone(chord, chromosome):
    """
        Calculates the decrease in fitness due to invalid successions
        of stable and unstable notes
    """
    fitness = 0
    
    if next_tone(chromosome, -1) not in stable_notes[chord]:
        fitness -= punishment_invalid_start
...
def fitness_note_leaps(chromosome):
    """
        Calculates the decrease or increase in fitness based on the amount
        of intervals of 1 or 2 (steps) and bigger intervals (leaps)
    """
